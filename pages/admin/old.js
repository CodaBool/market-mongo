...
export default function Index({ user }) {
  ...
  function getProducts() {
    axios.get('/api/product').then(res => {console.log(res.data); setProducts(res.data)}).catch(err => console.error('get err', err.response.data.msg))
  }
  function putProducts() {
    axios.put('/api/product', {_id: selector, data: {
      price: 155,
      quantity: 150,
    }}).then(res => setProducts([res.data])).catch(err => console.error('put err', err.response.data.msg))
  }
  function postProducts() {
    axios.post('/api/product', {
      _id: selector, // prod_IWYxENlgBE5gIx
      price: 100, 
      quantity: 100,
      name: 'New Valencia Product',
      images: ['https://files.stripe.com/links/fl_test_tbN5CakXUa74ea6hnFdPaiUs']
    }).then(res => setProducts([res.data])).catch(err => console.error('post err', err.response.data.msg))
  }
  function deleteProducts() {
    axios.delete('/api/product', { _id: selector }).then(res => console.log(res.data)).catch(err => console.error('del err', err.response.data.msg))
  }
  ...
  return (
    <div>
      ...
      <Card>
        <Card.Body>
          <h1 className="display-4 mr-5 d-inline">Price & Quantity</h1>
          <Button className="mx-2" onClick={getProducts}>Get All</Button>
          <Button variant="success" disabled={!selector} className="mx-2" onClick={postProducts}>Post</Button>
          <Button variant="warning" disabled={!selector} className="mx-2" onClick={putProducts}>Put</Button>
          <Button variant="danger" disabled={!selector} className="mx-2" onClick={deleteProducts}>Delete</Button>
          {products.length > 0 && products.map(product => (
            <div className="my-3" key={product._id}>
              {Object.keys(product).map((item, index) => 
                <p style={{lineHeight: '0'}} key={index}><strong>{item}:</strong> {typeof product[item] == 'object' ? 'obj' : product[item]}</p>
              )}
            </div>
          ))}
        </Card.Body>
