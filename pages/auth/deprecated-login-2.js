...
export default function newLogin({ providers }) {
  ...
  function emailChange(e) {
    setValidEmail(/[a-z0-9\._%+!$&*=^|~#%'`?{}/\-]+@([a-z0-9\-]+\.){1,}([a-z]{2,16})/.test(e.target.value))
        ...
        >
          <Card className="shadow" style={{border: 'none'}}>
            ...
            <Accordion.Collapse eventKey="0">
              <Card.Body className="d-flex" style={{justifyContent: 'center', flexDirection: 'column', flexWrap: 'nowrap', minHeight: '16rem'}}>
                  ...
                  <>
                    {Object.values(providers).map(provider => (
                      <div key={provider.name} className="mx-auto my-2">
                        <Button onClick={() => handleSignIn(provider.id)} disabled={!validEmail} style={{width: '10em'}}>{provider.name}</Button>
                      </div>
                    ))}
